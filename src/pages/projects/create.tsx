...
const projectTypes = [
  { id: 'novel', name: '小说', description: '长篇小说、短篇故事、连载作品', icon: BookOpen, color: '#9333EA' },
  { id: 'article', name: '文章', description: '系列文章、专栏、博客', icon: FileText, color: '#3B82F6' },
  { id: 'code', name: '代码', description: '代码项目、技术文档', icon: Code, color: '#22C55E' },
  { id: 'other', name: '其他', description: '其他类型的项目', icon: Folder, color: '#6B7280' },
]
  ...
  return (
    <View className={`min-h-screen bg-white dark:bg-black ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <View className="flex flex-row items-center justify-between px-4 py-4 border-b border-gray-200 dark:border-gray-800">
        <View onClick={() => Taro.navigateBack()} className="p-2 cursor-pointer active:opacity-60">
      ...
      <ScrollView className="p-4" scrollY style={{ height: 'calc(100vh - 150px)' }}>
            ...
            {projectTypes.map((t) => {
              ...
              return (
                <View
                  ...
                  className={`relative p-4 rounded-xl cursor-pointer active:opacity-80 transition-all ${
                    isSelected 
                      ? 'bg-blue-50 dark:bg-blue-900 dark:bg-opacity-30 border-2 border-blue-500' 
        ...
        {/* 提示 */}
        <View className="bg-blue-50 dark:bg-blue-900 dark:bg-opacity-30 rounded-xl p-4 mb-6">
          <Text className="block text-sm text-blue-600 dark:text-blue-400">
